...
console.log();
console.log(` Creating a new TailwindCSS Next app in ${chalk.green(rootFolder)}. `);
console.log();

// This uses --no-tags and --depth 1 in order to make the cloning faster
run("git", ["clone", "--no-tags", "--depth", "1", "https://github.com/thamognya/Template-TailWindCSS-Next-App.git", gitFolder]);

// Copies the yew-app folder
cpr(path.join(gitFolder, "src"), folderName, {}, function (err, files) {
  ...
  if (err) {
    ...
    // Install wasm-pack
    console.log(" Installed dependencies done ");
    console.log();
